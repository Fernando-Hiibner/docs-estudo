...
class App:
    ...
    def showDump(self):
        ...
        with open("Teste.txt", "w", encoding="utf-8") as oi:
            for key, value, index in self.text.dump("1.0", END, image=True):
                imageTkVerson = tk.PhotoImage(self.text.image_cget(index, 'image'))
                #image = Image.(imageTkVerson, (imageTkVerson.width(), imageTkVerson.height()))
                #print(type(image))
                oi.write(value)
                #image.save(value+".png")
                imageTkVerson.write(value+".gif", format="gif", from_coords=(0,0))
            oi.close()
    def testeRead(self):
